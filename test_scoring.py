...
def raw_value_for_effective_score(question_id, effective_value):
    """Traduce el valor efectivo al valor Likert visible."""
    if question_id in REVERSE_SCORED_IDS:
        return 6 - effective_value
...
class ScoringTests(unittest.TestCase):
    ...
    def test_all_answers_at_one_give_positive_x_only(self):
    ...
    def test_all_answers_at_five_give_negative_x_only(self):
        expected = {axis_id: 0.0 for axis_id in AXES}
        expected["x"] = -100.0
        self.assertEqual(
        ...
        for target_axis, axis in AXES.items():
            with self.subTest(axis=target_axis):
                ...
                for question_id in axis["positive"]:
                    answers[question_id] = raw_value_for_effective_score(
                        question_id,
                        5,
                    )
    ...
    def test_result_matches_a_manual_calculation(self):
        # Grupo positivo efectivo: ((6 - 5) + (6 - 4)) / 2 = 1.5
        # Grupo contrario efectivo: (2 + 4) / 2 = 3.0
        # ((1.5 - 3.0) / 4) * 100 = -37.5
        answers = answers_with(3)
